...
audio_path = "../results/tts_spanish_output.wav"

print("Transcribing Spanish audio...")

with open(audio_path, "rb") as audio_file:
    response = client.stt.transcribe(
        file=("tts_spanish_output.wav", audio_file, "audio/wav"),
        model="ink-whisper",
        language="es",
    )
...
with open("../results/stt_spanish_result.txt", "w", encoding="utf-8") as f:
    f.write(f"Transcription: {response.text}\n")
    f.write(f"Language: {response.language}\n")
    f.write(f"Duration: {response.duration}s\n")

print("✅ Result saved to ../results/stt_spanish_result.txt")